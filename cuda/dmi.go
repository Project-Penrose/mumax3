...
// See dmi.cu
func AddDMI(Beff *data.Slice, m *data.Slice, Aex, Dex SymmLUT, Msat MSlice, regions *Bytes, mesh *data.Mesh, OpenBC bool) {
	...
	k_adddmi_async(Beff.DevPtr(X), Beff.DevPtr(Y), Beff.DevPtr(Z),
		...
		unsafe.Pointer(Aex), unsafe.Pointer(Dex), regions.Ptr,
